...
static int sp;    // REG  栈指针: 指向栈中下一个最顶的基址
static int local; // REG  指向当前VM函数local的基址
static int argument; // REG  指向当前VM函数argument段的基址
static int _this;    // REG  指向当前this段(在堆中)的基址
static int that;     // REG  指向当前that段(在堆中)的基址
static int ip;       // REG  指向下一条要执行的指令
static int temp[7];  // REG  存储临时值
/*
 *       RAM ADRRESS        FUNCTION
 *          0~15            No use，to be uesd
 *        16~155            vm's static value
 *      256~2047            stack
 *    2048~16383            heap to store obj & array
 **/
static short ram[266385];                       // 数据存储器
static vector<vector<string>> instructions_ram; // 指令存储器
static int staticCount; // 记录静态变量已经分配的数量
static unordered_map<string, int> staticVarNames; // 记录静态变量在内存中的位置  POINTER
...
void executeGoto(const string &label)
...
void executeFunction(const string &functionName, int numLocals)//To allocate the vars
{
