...
void Analyzer::checkExpression(Parser::TreeNode *t)
{
    if (t != nullptr)
    {
        ...
        switch (t->nodeKind)
        {
            case Parser::VAR_K: //To search in the func table
            {
                SymbolTable::Info info =
                        symbolTable->subroutineTableFind(t->token.tokenValue);
                if (info == SymbolTable::None)
                {
                    info = symbolTable->classesTableFind(currentClassName, t->token.tokenValue);
                    if (info == SymbolTable::None)
                    {
                        error5(currentClassName, t->token.row, t->token.tokenValue);
                    }
                }
            }
                break;
            case Parser::ARRAY_K:
            {
                ...
                if (info.type != "Array")
                {
                    error6(currentClassName, t->token.row, t->token.tokenValue);
                }
            }
                break;
            ...
            {
                if (t->token.tokenValue.find('.') ==
                    string::npos) // call_statement -> ID ( expressions )
                {
                    // 先检查函数有没有在当前类中声明
                    string functionName = t->token.tokenValue;
                ...
                {
                    // 先检查caller
                    string callerName = Parser::getCallerName(t->token.tokenValue);
                    ...
                    } else // 如果调用者是对象
                    {
                        ...
                        t->child[0]->nodeKind = Parser::METHOD_CALL_K;
                        //                    t->token.tokenValue = objInfo.type + "." +
                        //                    functionName;
                    }
                }
                break;
    ...
    switch (t->nodeKind)
    {
        ...
        case Parser::RETURN_STATEMENT_K:
        {
            ...
            if (info.kind == SymbolTable::CONSTRUCTOR &&
                t->child[0]->token.tokenValue != "this")
            {
