...
void SymbolTable::classesTableInsert(Parser::TreeNode *t)
{
    if (t->nodeKind == Parser::CLASS_K)
    {
        ...
        currentClass = t->child[0]->token.tokenValue;
    ...
    {            // t->child[0] = static | field
        ...
        info.type =
                t->child[1]
                        ->token.tokenValue; // t->child[2] = varName - varName - varName ...
        for (auto p = t->child[2]; p != nullptr; p = p->next)
        {
            string name = p->token.tokenValue;
            if (t->child[0]->token.tokenValue == "field")
            {
                info.kind = FIELD;
                info.index = field_index++;
            } else if (t->child[0]->token.tokenValue == "static")
            {
                info.kind = STATIC;
    ...
    {                                                   // t->child[0] = function
        Info info;                                        // t->child[1] = type
        if (t->child[0]->token.tokenValue == "function") // t->child[2] = functionName
            info.kind = FUNCTION; // t->child[3] = arg - arg - arg ...
        else if (t->child[0]->token.tokenValue == "method")
            info.kind = METHOD;
        else if (t->child[0]->token.tokenValue == "constructor")
            info.kind = CONSTRUCTOR;
        info.type = t->child[1]->token.tokenValue;
        for (auto p = t->child[3]; p != nullptr; p = p->next)
        {
            string type = p->child[0]->token.tokenValue;
            info.args.push_back(type);
        }
        string name = Parser::getFunctionName(t->child[2]->token.tokenValue);
...
void SymbolTable::subroutineTableInsert(Parser::TreeNode *t)
{
    ...
    {                                                 // t->child[0] = function
        ...
        string className = Parser::getCallerName(
                t->child[2]->token.tokenValue); // t->child[2] = functionName
        string functionName = Parser::getFunctionName(
                t->child[2]->token.tokenValue); // t->child[3] = arg - arg - arg ...
    ...
    {                                          // t->child[0] = type
        ...
        info.type = t->child[0]->token.tokenValue;
        if (info.type != "int" && info.type != "char" && info.type != "void" &&
            info.type != "string" && info.type != "boolean") // 如果不是基本类型
        {
        ...
        ofstream symbolTableTest;
        symbolTableTest.open("SymbolTable.txt", ios::trunc);
        ...
        symbolTableTest << "class index: " << endl;
        symbolTableTest << "类名\t\t编号\t\t" << endl;
        for (auto iter = classIndex.cbegin(); iter != classIndex.cend(); ++iter)
            symbolTableTest << iter->first << "\t\t" << iter->second << endl;
        symbolTableTest << endl;
        symbolTableTest << "********************符号表********************" << endl;
        for (int i = 0; i < classesTable.size(); i++)
        {
            symbolTableTest << "class table: " << i << endl;
            symbolTableTest << "name\ttype\tkind\tvars" << endl;
            for (auto iter = classesTable[i].cbegin(); iter != classesTable[i].cend();
                 ++iter)
            {
                symbolTableTest << iter->first << "\t" << iter->second.type << "\t"
                                << iter->second.kind << "\t" << iter->second.index;
                for (int k = 0; k < iter->second.args.size(); ++k)
                    symbolTableTest << iter->second.args[k] << "\t";
                symbolTableTest << endl;
            }
            symbolTableTest << endl << endl;
        }
        // 再检查varName是否合理
        string varName = t->child[1]->token.tokenValue;
    ...
    {                                            // t->child[0] = type
        ...
        {
            string varName = p->token.tokenValue;
...
{
    ofstream symbolTableTest;
    symbolTableTest.open("SymbolTable.txt");
